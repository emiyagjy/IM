...
{
	return [[XMPPPresence alloc] initWithType:type to:nil id:nil];
}

// 改 添加id
+ (XMPPPresence *)presenceWithType:(NSString *)type to:(XMPPJID *)to withId:(NSString *)strId
{
	return [[XMPPPresence alloc] initWithType:type to:to id:strId];
}
...
{
	return [self initWithType:type to:nil id:nil];
}
...
- (id)initWithType:(NSString *)type to:(XMPPJID *)to id:(NSString *) strId;
{
	if ((self = [super initWithName:@"presence"]))
	{
        ...
        if (strId) {
            [self addAttributeWithName:@"id" stringValue:strId];
        }
