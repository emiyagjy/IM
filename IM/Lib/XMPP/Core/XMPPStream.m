...
- (void)continueSendPresence:(XMPPPresence *)presence withTag:(long)tag
{
	...
	//LOG(@"----------- Send Presence -----------: %@",outgoingStr);
    
    XMPPLogSend(@"----------- SEND Presence -----------: %@", outgoingStr);
...
- (void)sendOpeningNegotiation
{
    ...
    else
    {
		if (myJID_setByClient)
		{
            ...
            // 改
//            temp = @"<stream:stream xmlns='%@' xmlns:stream='%@' version='1.0' to='%@@%@'>";
//            s2 = [NSString stringWithFormat:temp, xmlns, xmlns_stream,[myJID_setByClient user],[myJID_setByClient domain]];
...
- (void)handleStreamFeatures
{
	...
	if (f_bind)
	{
		...
		if ([requestedResource length] > 0)
		{
			...
			// LOG(@"----------- BIND SEND -----------: %@", outgoingStr);
            XMPPLogSend(@"----------- BIND SEND -----------:%@", outgoingStr);
			numberOfBytesSent += [outgoingData length];
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag
{
	...
	//LOG(@"————----————---- RECV ————----————----: %@", [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]);
    
    XMPPLogRecvPre(@"————----————---- RECV ————----————----: %@", [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]);
    
    
	
	//NSString *dataString =@"<?xml version='1.0' encoding='UTF-8'?> <stream:stream xmlns:stream=\"http://etherx.jabber.org/streams\" xmlns=\"jabber:client\" from=\"10.200.2.6\" id=\"3e88e88\" xml:lang=\"en\" version=\"1.0\">";
    
    //[[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
//    dataString=[dataString stringByReplacingOccurrencesOfString:@"xmlns" withString:@"noNSxml"];
...
//    
	...
	else
	{
		...
		//b We'll read more data after we've parsed the current chunk of data.
